# -*- coding: utf-8 -*-
"""
Created on Sat May  4 00:20:03 2024
...
# Read Excel file
df = pd.read_excel(file_path, "Spot_NO_VA_shock_DOWN", usecols="C", )
df = df[11:]
...
EQt_rt_down = simulate_gbm_vec(F0, rt_down, sigma_equity, dt, N, m_MC)
PRt_rt_down = simulate_gbm_vec(F0, rt_down, sigma_property, dt, N, m_MC)
...
print(str(tt.shape) + str(fund_value_rt_down.T.shape))

plt.plot(tt, fund_value_rt_down.T);
plt.xlabel("Years $(t)$")
plt.ylabel("Portfolio Value $(F_t)$")
plt.title("Portfolio value (€) VS time (year)\n $F_t = EQ_t_rt_down + PR_t_rt_down$, $F_0 = 100000€$")
plt.show()
...
# MC :
MC_liab = MC_lapse_liabilities(m_MC, F_rt_down, discount_factors, px_cumul[:50], lapse_rate, rd_rate)
MC_liab
...
proba = proba[:50]
what = lapse_what_do_you_get(F_rt_down[0], discount_factors)
...
# MC :
d_MC_liab = MC_death_liabilities(m_MC, F_rt_down, discount_factors, px_cumul[:50], lapse_rate, rd_rate)
d_MC_liab
...
d_proba = d_proba[:50]
d_what = death_what_do_you_get(F_rt_down[0], discount_factors, F0)
...
def L_and_D_COMM_Liabilities_MC(m_MC, COMM, N):
    ...
    for m in range(m_MC):
        # Calculate what you get when you die
        death_what_you_get = death_what_do_you_get(F_rt_down[m], discount_factors, F0)
        # Calculate what you get when you lapse
        lapse_what_you_get = lapse_what_do_you_get(F_rt_down[m], discount_factors)
...
BOF_rt_down = F_rt_down[-1] - TOT_Liabilities # a corriger !
BOF_rt_down

#Supposing that BOF (BOF whithout shock) was computed before

d_BOF_rt_down = BOF - BOF_rt_down
d_BOF_rt_down

SCR_rt_down = max(0, d_BOF_rt_down)
SCR_rt_down
