# -*- coding: utf-8 -*-
"""
Created on Sat May  4 00:14:19 2024
...
Life_table['Death probability qx'] *= 1.15
Life_table_shock = Life_table
px_cumul= Life_table_shock["Cumulative survival probability px"]*(1-Life_table['Death probability qx'])

# Life table (taken from the real italian life table)
life_table_shock = Life_table_shock['Death probability qx']

# Initialize lists to store population data
population_in_contract_shock = [initial_population]
population_lapsed = [0]
population_dead_shock = [0]
...
for year in range(1, N + 1):
    ...
    deaths_shock = np.random.binomial(population_in_contract_shock[-1], life_table_shock[year - 1])

    # Calculate number of lapses
    lapses = int(population_in_contract_shock[-1] * lapse_rate)

    # Update population
    population_in_contract_shock.append(max(0, population_in_contract_shock[-1] - deaths_shock - lapses))
    population_lapsed.append(population_lapsed[-1] + lapses)
    population_dead_shock.append(population_dead_shock[-1] + deaths_shock)

# Plot population evolution
years_range = range(N + 1)
plt.plot(years_range, population_in_contract_shock, label='In Contract')
plt.plot(years_range, population_lapsed, label='Lapsed')
plt.plot(years_range, population_dead_shock, label='Dead')
plt.xlabel('Years')
...
# px_n = px_n-1*(1-qx_n-1_n)  
    
#px_cumul = Life_table_shock["Cumulative survival probability px"]
...
qx_shock = Life_table_shock["Death probability qx"]

def death_proba(px_cumul, lapse_rate, qx_shock):
    """
    """
    death_p_shock = np.zeros(px_cumul.shape)
    for k in range(1, len(px_cumul)):
        if k == 1:
            death_p_shock[k] = 1 * (1 - lapse_rate) ** (k - 1) * qx_shock[k - 1]
        else:
            death_p_shock[k] = px_cumul[k - 2] * (1 - lapse_rate) ** (k - 1) * qx_shock[k - 1]
    return death_p_shock
...
def death_liabilities(RD, death_proba_shock, death_what_do_you_get):
    """
    """
    return (1 - RD) * death_proba_shock * death_what_do_you_get
...
def MC_death_liabilities(m_MC, F, discount_factors, px_cumul, lapse_rate, RD):
    ...
    death_p_shock = death_proba(px_cumul, lapse_rate, life_table_shock)

    sum = 0

    for m in range(m_MC):
        # Calculate what you get when you die
        death_what_you_get_shock = death_what_do_you_get(F[m], discount_factors, F0)

        # Calculate death liabilities
        death_liab_shock = death_liabilities(RD, death_p_shock, death_what_you_get_shock)
        sum += death_liab_shock
    
    # Calculate the death liabilities using Monte Carlo simulation
    death_liab_MC_shock = sum / m_MC
    return np.array(death_liab_MC_shock)

# Example from the death liability :

# MC :
d_MC_liab_shock = MC_death_liabilities(m_MC, F, discount_factors, px_cumul[:50], lapse_rate, rd_rate)
d_MC_liab_shock

d_MC_liab_shock.sum()

d_proba_shock = death_proba(px_cumul, 0.15, qx_shock)
d_proba_shock = d_proba_shock[:50]
d_what_shock = death_what_do_you_get(F[0], discount_factors, F0)

Death = pd.DataFrame()
Death["Proba Death Shock"] = d_proba_shock
Death["Discounted Present Value of the Death Cash Flows Shock"] = d_what_shock
Death["Death Liabilities Shock"] = d_MC_liab_shock
Death.head()


# Computation of the lapse and death commission liabilities :

def L_and_D_COMM_Liabilities(d_proba_shock, l_proba, d_DPV, l_DPV, COMM):
    d_COMM_liabilities = d_proba_shock * d_DPV * COMM
    l_COMM_liabilities = l_proba * l_DPV * COMM
    return np.array(d_COMM_liabilities), np.array(l_COMM_liabilities)

b = L_and_D_COMM_Liabilities(d_proba_shock, proba, d_what_shock, what, COMM)
b[0].sum() + b[1].sum()
...
def L_and_D_COMM_Liabilities_MC(m_MC, COMM, N):
    ...
    """
    # Calculate death probabilities
    death_p_shock = death_proba(px_cumul, lapse_rate, life_table_shock)[:N]
    # Calculate lapse probabilities
    ...
    for m in range(m_MC):
        ...
        L_and_D_COMM_Liab = L_and_D_COMM_Liabilities(death_p_shock, lapse_p, death_what_you_get, lapse_what_you_get, COMM)
...
summary = liabilities_summary(MC_liab, d_MC_liab_shock, L_n_D_COMM_liab, Exp_liab, N)
summary

TOT_Liabilities = summary["Total per year"].sum()
TOT_Liabilities

BOF_death_proba_shock = F[-1] - TOT_Liabilities # a corriger !
BOF_death_proba_shock

#Supposing that BOF (BOF whithout shock) was computed before

d_BOF_death_proba_shock = BOF - BOF_proba_shock
d_BOF_death_proba_shock

SCR_death_proba_shock = max(0, d_BOF_death_proba_shock)
SCR_death_proba_shock
