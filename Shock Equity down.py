# -*- coding: utf-8 -*-
"""
Created on Sat May  4 00:07:28 2024
...
EQt_down = simulate_gbm_vec(F0, rt, sigma_equity, dt, N, m_MC)*(1-shock_rate_equity_down)
...
print(str(tt.shape) + str(fund_value_eqt_down.T.shape))

plt.plot(tt, fund_value_eqt_down.T);
plt.xlabel("Years $(t)$")
...
# MC :
MC_liab = MC_lapse_liabilities(m_MC, F_eqt_down, discount_factors, px_cumul[:50], lapse_rate, rd_rate)
MC_liab
...
proba = proba[:50]
what = lapse_what_do_you_get(F_eqt_down[0], discount_factors)
...
# MC :
d_MC_liab = MC_death_liabilities(m_MC, F_eqt_down, discount_factors, px_cumul[:50], lapse_rate, rd_rate)
d_MC_liab
...
d_proba = d_proba[:50]
d_what = death_what_do_you_get(F_eqt_down[0], discount_factors, F0)
...
def L_and_D_COMM_Liabilities_MC(m_MC, COMM, N):
    ...
    for m in range(m_MC):
        # Calculate what you get when you die
        death_what_you_get = death_what_do_you_get(F_eqt_down[m], discount_factors, F0)
        # Calculate what you get when you lapse
        lapse_what_you_get = lapse_what_do_you_get(F_eqt_down[m], discount_factors)
...
BOF_eqt_down = F_eqt_down[-1] - TOT_Liabilities # a corriger !
BOF_eqt_down

#Supposing that BOF (BOF whithout shock) was computed before

d_BOF_eqt_down = BOF - BOF_eqt_down
d_BOF_eqt_down

SCR_eqt_down = max(0, d_BOF_eqt_down)
SCR_eqt_down